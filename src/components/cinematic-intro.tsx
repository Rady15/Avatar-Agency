...
export default function CinematicIntro({ onEnter }: { onEnter: () => void }) {
  ...
  useEffect(() => {
    if (showVideo && videoRef.current) {
      videoRef.current.play().catch(() => {
        // If autoplay fails, just proceed
        handleVideoEnded();
      });
    }
  }, [showVideo]);
  ...
  return (
    <div className={`fixed inset-0 z-[9999] bg-black transition-opacity duration-1000 ${isExiting ? 'opacity-0' : 'opacity-100'}`}>
      {/* Loading overlay */}
      {!isReady && (
      ...
      {/* 3D Canvas - hide when video is playing */}
