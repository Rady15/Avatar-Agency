...
export function ServiceVerticalStack({ items, accentColor = "#D4AF37" }: ServiceVerticalStackProps) {
  ...
  if (items.length === 0) {
    return (
      <div className="flex h-[500px] w-full items-center justify-center text-muted-foreground">
  ...
  return (
    <div className="relative flex h-[600px] w-full items-center justify-center overflow-hidden bg-transparent">
      ...
      <div 
        className="relative flex h-[550px] w-[400px] md:w-[480px] items-center justify-center" 
      ...
      >
        {items.map((item, index) => {
          ...
          return (
            ...
            >
              <div
                className="relative w-[380px] md:w-[460px] overflow-hidden rounded-3xl bg-card ring-1 ring-border/20 bg-black/20"
                style={{
                  boxShadow: isCurrent
              ...
              >
                <div 
                  className="absolute inset-0 rounded-3xl bg-gradient-to-b from-white/10 via-transparent to-transparent"
                  style={{ 
                    background: `linear-gradient(to bottom, ${accentColor}15 0%, transparent 50%)` 
                  }}
                />
        ...
        {items.map((_, index) => (
          <button
            ...
            className={`h-2 w-2 rounded-full transition-all duration-300 ${
              index === currentIndex ? "h-6" : "bg-foreground/30 hover:bg-foreground/50"
            }`}
            style={{ 
      ...
      <div className="absolute left-8 top-1/2 -translate-y-1/2">
        <div className="flex flex-col items-center">
          ...
          />
          <span className="text-sm text-muted-foreground tabular-nums">{String(items.length).padStart(2, "0")}</span>
        </div>
      </div>
    </div>
  )
